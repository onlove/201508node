...
http.createServer(function(req,res){
    ...
    //etagHanndle(filename,req,res);
    expireHandler(filename,req,res);
}).listen(8081);


/*
* last-Modified  最后修改时间 服务器发给客户端
* if-Modified-since 在最后的修改时间之后，是否有新的修改，客户端发给服务器
* 1.第一次客户端访问服务器，服务器会把此文件的修改时间和文件内容一起返回
* 2.客户端收到文件后，会记录文件的最后修改时间并且缓存此文件
* 3.当客户端再次需要此文件的时候，会把此文件的最后修改时间发回服务器(if-Modified-since)
* 4.服务器进行判断是修改时间之后有没有修改过
*      没修改  返回304 告诉客户端缓存可用
*      修改   返回最新的文件内容和最新的修改时间
* */
...
* etag是服务器生成的，发送给客户端，客户端不关心etag怎么生成
*
* 1.第一次服务器生成etag发给客户端
...
function etagHanndle(filename,req,res){
...
function expireHandler(filename,req,res){
    fs.readFile(filename,function(err,data){
        res.setHeader('Expires',new Date(new Date().getTime()+20*1000).toGMTString())
